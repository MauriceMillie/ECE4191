#!/usr/bin/env python3
"""
ECE4191 Module 3 -- Node-646 Toy QP over Modbus TCP
===================================================

A single-node (Node 646) reduction of ``central_qp_modbus.py`` for
"triangle / square" unit test.

Toy setup
---------
* Input CSV is the 2-row wide toy file (signals as rows, time steps as columns):

      <label>,   0, 8.33, ... , 8.33        <- P_PV_kW  (triangle 0->200->0)
      P_load_kW, 0, 0,    ... , 0           <- P_load_kW (square 150 kW block)

  96 columns = 2 days x 48 half-hour steps. The loader is tolerant of the
  stray '\\' label on the PV row and of an optional step-index header row.

* Only Node 646 is driven. Holding registers 2000-2003, "normal" order
  [Pref, Qref, PV, Batt]. All other BusSplitMap registers are cleared to 0.

* Node 646: 102 customers -> 1020 kWh battery capacity.
...
* The ACTUAL CSV (``--actual``) supplies the load/PV that are written to
  Node 646, and the optimised grid power is recomputed as
...
Typical run
-----------
    # fast offline check, no HIL:
    python3 central_qp_modbus_toy_n646.py --input toy_example_N646_students.csv \
        --dry-run --no-wait

    # live against the HIL, logging measured SoC:
    python3 central_qp_modbus_toy_n646.py --input toy_example_N646_students.csv \
        --no-prompt
"""
...
def main():
    ...
    pv_days, load_days, n_days = load_toy_csv(csv_path)
    print(f"Loading actual   CSV: {act_path.name}")
    pv_act_days, load_act_days, n_days_act = load_toy_csv(act_path)
