...
MPC-CIL at node 646. (NO BATTERY COMMANDS SENT TO OTHER NODE BATTERIES UNLIKE IN EXPERIMENT 2)
"""
...
def solve_daily_qp(p_load, p_pv, eta, weight, batt_power_kw, capacity_kwh,
    ...
    objective = cp.Minimize(cp.sum(-DELTA_HOURS * cp.multiply(eta, batt) + weight * cp.multiply(eta, cp.square(grid)))
    )
...
def read_node646_soc_pct(conn) -> float | None:
    """Read Node 646's measured SoC (%) from register 3000."""
    try:
    ...
    try:
        for i in range(total_steps):
            ...
            if status not in ("optimal", "optimal_inaccurate"):
                print(f"  WARNING: MPC status = {status} at step {step}; applying zero battery command for this step.")
                p_bat_646 = 0.0
            ...
            # CIL feedback: read the REAL Node 646 SoC from typhoon hil and use it as SoC(t|t) for the next MPC solve
