...
# Allow dry test on a machine without pymodbus
try:
...
# Default weight value if no arg is entered 
DEFAULT_WEIGHT = 1

# Aggregate feeder battery per customer info:
...
# Customer counts for battery disaggregation.
N_CUSTOMERS = {
    "646_B": 102,
    "645_B": 63,
    "611_C": 68,
...
def solve_daily_qp(
    ...
    # soc = cp.Variable(n + 1, name="SoC") # old. dont uncomment
    ...
    constraints = [
        # OLD CONSTRAINTS (KEEP COMMENTED)
...
class SocLogger:
    # Read Node 646 SoC for logging only; dont feed it back in Experiment 1.
    ...
    def start(self) -> None:
        ...
        try:
            ...
            print(
                f"  WARNING: Node 646 SoC could not be read ({exc}). Measured SoC will be left blank."
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "ECE4191 Module 3 Experiment 1 - feeder-wide day-ahead QP battery playback over Modbus TCP."
        )
    )
    ...
    parser.add_argument(
        "--output",
        default="Output/experiment1_qp_schedule.csv",
        ...
        with output_path.open("w", newline="", encoding="utf-8") as fh:
            ...
            for day_i, date_label in enumerate(forecast_dates):
                ...
                for k, time_label in enumerate(forecast_time_cols):
                    ...
                    # Let the plant update before sampling SoC; the step duration is either equal or roughly equal to the time step that is set
                    if (
                        not args.no_wait
