...
%
% Experiment 2 therefore lasts:
%       192 steps x 2 real s/step = 384 real seconds
%
...
% Automatic alignment is recommended for sequential HIL panel exports.
...
% Calculated automatically below after inspecting every panel CSV.
...
% Only the seven full Experiment 2 HIL panel exports are used here.
% The MPC schedule CSV is NOT included because it is the controller log,
% not a sequentially exported Signal Analyzer panel.
panelFiles = [ ...
    string(powerPath1); ...
    string(powerPath2); ...
    string(feederPath); ...
...
% Because the panels were exported one after another while the simulation
% was still running, the first panel exported should have the earliest
% final Time value.
...
if PLAYBACK_START_REAL_S < 0

    error([ ...
        'Inferred playback start is negative. The earliest panel does not ' ...
        'contain the full known Experiment 2 playback duration.']);
end

% Verify that every panel contains the common inferred window.
...
if any(panelAlignment.EndTime_s < PLAYBACK_END_REAL_S - 1e-9)
    ...
    error([ ...
        'At least one HIL panel ends before the inferred playback end. ' ...
        'Automatic alignment cannot safely create a common window.']);
end

% Estimate the amount of priming/dummy recording before playback.
commonRecordingStartS = min(panelAlignment.StartTime_s);

primingRealSeconds = ...
    PLAYBACK_START_REAL_S - commonRecordingStartS;

simHoursPerRealSecond = ...
    SIM_HOURS_PER_STEP / REAL_SECONDS_PER_STEP;

primingSimHours = ...
    primingRealSeconds * simHoursPerRealSecond;
...
alignmentCsv = fullfile( ...
    outputDir, ...
    "Experiment2_Panel_Time_Alignment.csv");
